...
3. アカウント/アプリに以下の権限を付与:
   - **財務データ、注文、キャンセル調査レポートの閲覧**（読み取り・RevenueCat購入検証用）
   - **ストアでの表示の管理（Manage store presence）** ← **サブスク/アプリ内商品の作成・更新・有効化に必須**
     （実API検証 2026-06-26: これが無いと作成が `403 PERMISSION_DENIED`。「注文と定期購入の管理」だけでは作成不可）
4. 反映に時間がかかる場合があります。`403 permission denied` が出たら権限（特にストア表示の管理）と反映待ちを確認。

> **regionsVersion 必須（実API検証 2026-06-26）**: サブスク作成は `regionsVersion.version`（既定 `2022/01`）を
> **クエリパラメータ**で渡す必要がある（body フィールドではない）。本サーバの `gpc_create_subscription` は
> 既定 `2022/01` を自動付与する（引数 `regionsVersion` で上書き可）。

...
## ▶ RESUME / 現在地（2026-06-29・/clear後はここから読む）
- **🧭 即・次アクション（2トラック並行）**: **【Android】署名付きAAB完成済**(`C:\Users\user\Downloads\Moffy-AAB\app-release.aab`/再ビルドは`gh workflow run "Build AAB"`)→**ユーザーがPlay内部テストにUL→サブスク作成解禁**→2商品作成→RevenueCat紐付け（=Androidは**ユーザー操作待ちで私は進めない**）。**【iOS】Screen Timeフル実装=コード完成＋クロスレビュー反映＋push済(下記🍎)。pbxproj配線=`Configure iOS`(Linux/無料)で🟢実証**。**🔴 iOS最優先ブロッカー＝既存依存衝突(Screen Timeと無関係)**: `device_info_plus 12.4.0`(推移的)は新iOS SDK(Xcode26)必須・`sentry_flutter 8.9.0`はXcode26で壊れる→同一Xcodeで両立せず**macOSコンパイル不能**。**修正(別タスク)=sentry_flutterをXcode26対応版へ更新(本筋)or device_info_plusをdependency_overridesで旧版固定+Xcode16**→その後macOS実コンパイル(=Swift唯一の本検証)。**ユーザー作業=Appleポータル準備(手順送付済・下記🍎残①)**。⚠️ユーザーはアプリ開発初心者・Mac無し・費用敏感。macOSビルドは計4回試行(依存衝突の切り分けに使用)。
...
- **🍎 iOS方針=フル実装で出す(2026-06-26ユーザー決定・日本市場iOS重視)**: **FamilyControls配布Entitlement=申請即承認済**(`com.moffy.app`/Apple Developer=Manabu Osawa・Lan LLC)。**ユーザーはMac無し**→iOSビルドはクラウドmacOS(GitHub Actions・10倍消費/少額費用注意)。
  ...
  - 🟥 **macOSコンパイル=既存依存衝突でブロック(Screen Time無関係・最優先で別タスク化)**: `device_info_plus 12.4.0`(推移的・lib未使用)が`isiOSAppOnVision`(新iOS SDK API)使用→Xcode16(iOS18 SDK)で`No visible @interface`失敗。`sentry_flutter 8.9.0→sentry-cocoa`はXcode26.3で`SentryBinaryImageCache has no member 'image'`失敗。同一Xcodeで両立不可。**修正候補**: ①`sentry_flutter`をXcode26対応版へ更新(本筋・observability(`crash_reporter.dart`/`main.dart`)へのAPI影響をCI(123テスト)で確認) or ②`dependency_overrides: device_info_plus`を旧版固定+Xcode16。修正は無料Linux CIで検証→最後にmacOS実コンパイル(=Swift唯一の本検証)。`ios-build.yml`は暫定latest-stable(Xcode26)。
  - 🔴 **iOSの残(順に)**: ①**【依存整合(別タスク)】**上記衝突を解消 ②有料(少額): `iOS Build`(macOS)でSwift実コンパイル検証 ③**Appleポータル準備(ユーザー作業・手順送付済・署名ビルド前必須)**: App ID 2つ(`com.moffy.app`/`com.moffy.app.MoffyMonitor`)/App Group `group.com.moffy.app`登録&両IDに付与/**Family Controls(配布)を拡張App IDにも申請(アプリ側は承認済・拡張は別App IDで別途・承認数週間)**/両IDのプロビジョニング ④署名/TestFlight(asc-mcpのASC APIキーをfastlaneで流用可)。
...
| 日付 | 決定 | 理由 |
|---|---|---|
| 2026-06-29 | 【CEO/QA】iOS Screen Timeをフル実装(コード)→API表面をWeb検証(high-conf)→Claude-QA独立3観点クロスレビュー(GO_WITH_FIXES)で指摘反映。macOS実コンパイルは有料ゲートでユーザーGO待ち。**シールド(アプリブロック)はしない=報酬型・観測のみ** | iOSは分数取得不可の別設計でMac無し→「実装前にAPI検証・実装後に別観点レビュー」で誤りを潰す(書いた本人が検品しない鉄則)。コンパイル=Swiftの唯一の本検証だが課金のため明示ゲート。シールド回避で2つ目の拡張/Entitlement申請を不要に |

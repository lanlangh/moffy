...
- **🧭 即・次アクション（2トラック並行）**: **【Android】署名付きAAB完成済**(`C:\Users\user\Downloads\Moffy-AAB\app-release.aab`/再ビルドは`gh workflow run "Build AAB"`)→**ユーザーがPlay内部テストにUL→サブスク作成解禁**→2商品作成→RevenueCat紐付け（=Androidは**ユーザー操作待ちで私は進めない**）。**【iOS】Screen Timeフル実装=コード完成＋クロスレビュー反映済(下記🍎)**。**🔴 iOSの次の一手＝(1)無料: push後 `gh workflow run "Configure iOS (Screen Time)"`(Linux)でpbxproj配線を無料検証→(2)有料判断: `gh workflow run "iOS Build (macOS)"`(macOS=約10倍課金)でSwift実コンパイル検証(=Swiftの唯一の本検証・ユーザーのGO待ち)→(3)署名/TestFlightはAppleポータル準備後**。⚠️ユーザーはアプリ開発初心者・Mac無し。**コミット/pushは未実施(ユーザー承認待ち)**。
...
- **🍎 iOS方針=フル実装で出す(2026-06-26ユーザー決定・日本市場iOS重視)**: **FamilyControls配布Entitlement=申請即承認済**(`com.moffy.app`/Apple Developer=Manabu Osawa・Lan LLC)。**ユーザーはMac無し**→iOSビルドはクラウドmacOS(GitHub Actions・10倍消費/少額費用注意)。
  ...
  - 🔴 **iOSの残(順に)**: ①無料: `Configure iOS`(Linux)で配線検証 ②有料(ユーザーGO待ち): `iOS Build`(macOS)でコンパイル検証＝Swiftの唯一の本検証 ③**Appleポータル準備(ユーザー作業・署名ビルド前必須)**: App ID 2つ(`com.moffy.app`/`com.moffy.app.MoffyMonitor`)/App Group `group.com.moffy.app`登録&両IDに付与/**Family Controls(配布)を拡張App IDにも申請(アプリ側は承認済・拡張は別App IDで別途・承認数週間)**/両IDのプロビジョニング ④署名/TestFlight(asc-mcpのASC APIキーをfastlaneで流用可)。

...
- **🍎 iOS方針=フル実装で出す(2026-06-26ユーザー決定・日本市場iOS重視)**: **FamilyControls配布Entitlement=申請即承認済**(`com.moffy.app`/Apple Developer=Manabu Osawa・Lan LLC)。**ユーザーはMac無し**→iOSビルドはクラウドmacOS(GitHub Actions・10倍消費/少額費用注意)。
  ...
  - 🔴 **iOSの残(順に)**: ①**Appleポータル準備(ユーザー作業・手順送付済・署名ビルド前必須)**: App ID 2つ(`com.moffy.app`/`com.moffy.app.MoffyMonitor`)/App Group `group.com.moffy.app`登録&両IDに付与/**Family Controls(配布)を拡張App IDにも申請(アプリ側は承認済・拡張は別App IDで別途・承認数週間)**/両IDのプロビジョニング ②`ios-build.yml`を署名archive/`flutter build ipa`化(現状は`--no-codesign`のコンパイル検証のみ) ③TestFlight(asc-mcpのASC APIキーをfastlaneで流用可) ④実機でしきい値到達→App Group記録→ポイント反映のE2E確認。

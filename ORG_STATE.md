...
- **🧭 即・次アクション（2トラック並行）**: **【Android】署名付きAAB完成済**(`C:\Users\user\Downloads\Moffy-AAB\app-release.aab`/再ビルドは`gh workflow run "Build AAB"`)→**ユーザーがPlay内部テストにUL→サブスク作成解禁**→2商品作成→RevenueCat紐付け（=Androidは**ユーザー操作待ちで私は進めない**）。**【iOS】Screen Timeフル実装=コード完成＋クロスレビュー反映＋push済(下記🍎)。pbxproj配線=`Configure iOS`(Linux/無料)で🟢実証**。**✅ iOS実コンパイル(macOS CI)=成功(2026-06-29)**＝Screen Time Swiftの本検証クリア。到達まで既存依存/ツールチェーン衝突(Screen Time無関係)を解消: `device_info_plus`を`dependency_overrides`で`12.3.0`固定(12.4.0は`isiOSAppOnVision`でXcode16不可)／`sentry_flutter 8.x`はSPM非対応→**Flutter SPM無効化=CocoaPods統一**／Xcode**16.x**固定(latest=26.3は2024年代プラグインを壊す)／`Embed App Extensions`を`Thin Binary`前へ移動(`Cycle inside Runner`回避)／自Swift修正(messenger()は非Optional・AuthorizationStatusに`.approvedWithDataAccess`無し)。**🔴 iOS残=ユーザー作業のAppleポータル準備(手順送付済・下記🍎残①)→署名archive/TestFlight**。⚠️ユーザーはアプリ開発初心者・Mac無し・費用敏感。macOSビルドは依存衝突切り分けで複数回試行(多くは無料Linux検証で代替)。
...
- **🍎 iOS方針=フル実装で出す(2026-06-26ユーザー決定・日本市場iOS重視)**: Apple Developer=Manabu Osawa・**合同会社Lan(Team ID=`JKPUV48L3V`)**。**2026-06-29ポータル実査＋整備＝完了**: 当初Identifiersに`com.moffy.app`未登録だった(既存はlifenote/tsuzuru/viralumeのみ)が、ユーザーが当セッションで整備完了: ✅App Group `group.com.moffy.app`作成／✅App ID `com.moffy.app`(App Groups＋Family Controls Dev/Dist)作成・グループ紐づけ／✅App ID `com.moffy.app.MoffyMonitor`(同上)作成・グループ紐づけ。**✅Family Controls (Distribution)=「Assigned」=アカウントに承認済み→配布申請(数週間待ち)不要**(Capability Requestsタブで確認)。**=iOS署名ビルドのポータル前提は全て充足**。残るはCI署名設定→TestFlight。**ユーザーはMac無し**→iOSビルドはクラウドmacOS(GitHub Actions・10倍消費/少額費用注意)。
  ...
  - 🟧 **CI署名設定=実装済・あと1点(2026-06-30)**: `ios-build.yml`にmode=compile/release/testflight追加。`xcodebuild archive`+`-exportArchive`＋`-allowProvisioningUpdates`＋ASC APIキー(Secret: ASC_KEY_ID/ASC_ISSUER_ID/ASC_KEY_P8_BASE64=既存asc-mcpキーを登録)。ExportOptions=app-store-connect/automatic。Info.plistに`ITSAppUsesNonExemptEncryption=false`。**✅Archive成功**(配布証明書・Runner/MoffyMonitor両プロファイル自動生成・Family Controls配布OK・拡張署名OK=難所突破)。**🔴Exportが`Cloud signing permission error`**=エクスポートのクラウド署名は**Adminロールのキーが必要**(現キーはApp Manager)。→**ユーザーがAdmin権限のASC APIキーを新規作成**→私がSecret差し替え→`mode=release`再実行で署名IPA完了見込み。教訓: コマンドラインで`CODE_SIGN_IDENTITY`を渡すとAutomatic署名と競合する→渡さない(修正済)。
  - 🔴 **iOSの残(順に)**: ①**Adminキー差し替え→`mode=release`で署名IPA(マイルストーン1)** ②ASCにアプリレコード作成(`com.moffy.app`・手動・API不可)＋`mode=testflight`でアップロード ③実機でしきい値到達→App Group記録→ポイント反映のE2E確認。

...
- **🍎 iOS方針=フル実装で出す(2026-06-26ユーザー決定・日本市場iOS重視)**: **FamilyControls配布Entitlement=申請即承認済**(`com.moffy.app`/Apple Developer=Manabu Osawa・Lan LLC)。**ユーザーはMac無し**→iOSビルドはクラウドmacOS(GitHub Actions・10倍消費/少額費用注意)。Dart側はiOS対応済(`UsageProvider`抽象/`ThresholdAchievementPointCalculator`/`UsageMode.thresholdAchievement`スタブ)。**`ios/`フォルダ未作成→`scaffold-ios.yml`で生成中**。残: ①iOS雛形(bundle id=com.moffy.app化)②Swift製Screen Time拡張(DeviceActivityMonitor/FamilyControls/App Group/MethodChannel)＋`IOSUsageProvider`③iOSビルド(macOS CI)＋署名(ASC APIキー/fastlane)＋TestFlight④iPhone実機テスト。⚠️Mac無しのScreen Time native開発は反復が重い→クラウドMac(MacinCloud等)検討余地あり。iOS Screen Time制約=分数取得不可・しきい値近似(決定ログ参照)。

...
- **🧭 clear後の即・次アクション（2トラック並行）**: **【Android】署名付きAAB完成済**(`C:\Users\user\Downloads\Moffy-AAB\app-release.aab`/再ビルドは`gh workflow run "Build AAB"`)→**ユーザーがPlay内部テストにUL→サブスク作成解禁**→2商品作成→RevenueCat紐付け。**【iOS】土台完成→核心=Screen Time実装(下記🍎)に着手**。⚠️ユーザーはアプリ開発初心者・Mac無し。直近は「iOS Screen Time実装に進めるか／Androidを先に仕上げるか」をユーザーに確認した所でclearに入った。
...
- **🍎 iOS方針=フル実装で出す(2026-06-26ユーザー決定・日本市場iOS重視)**: **FamilyControls配布Entitlement=申請即承認済**(`com.moffy.app`/Apple Developer=Manabu Osawa・Lan LLC)。**ユーザーはMac無し**→iOSビルドはクラウドmacOS(GitHub Actions・10倍消費/少額費用注意)。
  ...
  - 🔴 **核心=Screen Time実装(未着手・大きな1機能)**。**重要な仕様差(Androidの移植ではない)**: ①データ=iOSは**分数取得不可**、DeviceActivityの**しきい値達成のみ**(対象アプリもopaque tokenで識別不可・プライバシー設計)②**UX差=ユーザーがOSの`FamilyActivityPicker`で対象SNSを自分で選ぶ**(Moffyは4SNSを自動指定できない)→**iOSオンボーディング分岐が必要**③経済=`threshold-achievement`(既存`ThresholdAchievementPointCalculator`/`UsageMode.thresholdAchievement`に近似値を流す)。
  - **契約**: `lib/core/usage/usage_provider.dart`(`UsageProvider`: checkPermission/requestPermission/fetchDailyUsage/fetchUsageRange)。`lib/core/usage/usage_providers.dart`の`usageProviderProvider`で`Platform.isIOS`時に新規`IOSUsageProvider`を返す配線が要る(現状はAndroid/else=`UnsupportedUsageProvider`)。Android実装(`android_usage_provider.dart`+Kotlin`UsageStatsHandler`)がMethodChannelの参考。チャネル名=`AppConstants.usageChannel`。
  - **4ステップ(残)**: ①iOS設定(`Runner.entitlements`=`com.apple.developer.family-controls`/App Group=アプリ↔拡張共有/Info.plist)②**Swift**: DeviceActivityMonitor拡張(しきい値到達→App Groupに記録)＋メインアプリ(FamilyControls authorization要求/`FamilyActivityPicker`/DeviceActivityスケジュール/MethodChannelハンドラ)③Dart`IOSUsageProvider`+`usage_providers.dart`配線④iOSビルド(macOS CI)＋署名＋TestFlight。**拡張ターゲットのXcodeプロジェクト追加はローカルRuby無し→CIで`gem install xcodeproj`してスクリプト追加**。署名/TestFlightは**asc-mcpのASC APIキー(KEY_ID/ISSUER_ID/PRIVATE_KEY=`asc-mcp/merge-settings.cjs`に在り・⚠️鍵直書き)をfastlaneで流用可**。
  - ⚠️ Mac無し→Screen Time native開発は実機デバッグ不可・反復ごとmacOS CIビルド(10倍消費)で重い→**クラウドMac(MacinCloud月$20前後等)検討余地**。これは「ビルド/反復」フェーズの話で、コード実装フェーズは無料で進められる。

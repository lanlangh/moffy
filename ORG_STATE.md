...
## ▶ RESUME / 現在地（2026-07-02・/clear後はここから読む）
- **💰🌐 リポジトリを public 化(2026-06-30)＝全 GitHub Actions が無料・無制限**（macOS iOSビルドの10倍消費の悩みが消滅・Codemagic移行は不要に）。public化前に秘密スキャン実施＝**本物の鍵(.p8/keystore/service_role/DB URL)は現在も履歴も一切無し**（公開されるのはコード＋開発メモ＋公開前提の anon `sb_publishable_...`/URL/識別子のみ）。**無料Webプレビュー(appetize無料枠終了の代替)=`https://lanlangh.github.io/moffy/`**（`.github/workflows/web-preview.yml` が main push で自動ビルド&Pagesデプロイ・`FORCE_MOCK=true`でテストデータ入り＝育成2/保管3/図鑑7発見済）。Web対応済(`dart:io`→`defaultTargetPlatform`・共有は条件付きimport)。**ユーザーの見た目確認はこのURLを開くだけ**。
